...
-- Q6 - Provide a breakdown of the numbers of employees enrolled, not enrolled, and with 
...
-- Q8 - Get a table of country, number of employees in that country, and the average 
...
-- Q9 - Return a table containing each employees first_name, last_name, full-time 
-- equivalent hours (fte_hours), salary, and a new column effective_yearly_salary which 
...
-- Q11 - The expected_profit of an employee is defined as (48 * 35 * charge_cost - salary) 
-- * fte_hours, where charge_cost depends upon the team to which the employee belongs. Get 
-- a table showing expected_profit for each employee.
...
-- Q12 Get a list of the id, first_name, last_name, salary and fte_hours of employees in 
-- the largest department. Add two extra columns showing the ratio of each employee’s 
-- salary to that department’s average salary, and each employee’s fte_hours to that 
...
-- Q13 - Return a table of those employee first_names shared by more than one employee, 
...
-- Q14 - Have a look again at your table for core question 6. It will likely contain a 
-- blank cell for the row relating to employees with ‘unknown’ pension enrollment status. 
-- This is ambiguous: it would be better if this cell contained ‘unknown’ or something 
-- similar. Can you find a way to do this, perhaps using a combination of COALESCE() and 
...
-- Q15 - Find the first name, last name, email address and start date of all the employees 
...
-- Q16 - Use a CASE() operator to group employees who are members of committees into 
-- salary_class of 'low' (salary < 40000) or 'high' (salary >= 40000). A NULL salary 
-- should lead to 'none' in salary_class. Count the number of committee members in each 
-- salary_class.
